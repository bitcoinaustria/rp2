...
- **Swap basis-value verification stays on Kassiber.** RP2 emits the outgoing-leg zero-gain `GainLoss` and now enforces pairing across assets (see the swap carryover contract): every `at_swap_link=<id>` must appear on exactly one outgoing leg and one incoming leg on two different assets, or the run fails before accounting. What RP2 still does *not* verify is the *value* Kassiber sets on the incoming leg's `fiat_in_with_fee`: it trusts Kassiber to have computed `crypto_out_no_fee * neu_pool_avg_at_swap_time` correctly. A Kassiber bug in the running-average/basis computation would propagate silently through RP2. Closing that gap would require cross-asset pool-state awareness in RP2's accounting engine and is out of scope.

...
* Correctness fixes (review pass):
  ...
  * **Spekulationsfrist is now a calendar year, not 365 days.** `classify_disposal` compares Vienna calendar dates against the acquisition anniversary (§ 108 BAO) instead of `holding_days >= 365`, so a holding that spans a 29 February is no longer misreported as past the Frist; a disposal on the anniversary day is still inside the Frist (taxable). Classifiers (`classify_lot_regime`, Spekulationsfrist) now read `cost_basis_timestamp` so transferred Altvermögen keeps its original acquisition date.
  * **Swap neutrality requires `moving_average_at`.** The native runner now raises if an outgoing swap leg yields no carried basis (i.e. the configured method is `fifo`/`moving_average`), instead of silently realizing a taxable gain on a row `classify_disposal` still buckets as `NEU_SWAP`.

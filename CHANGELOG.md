...
* Correctness fixes (review pass):
  * **Earn events no longer corrupt the moving-average pool.** `TaxEngineCursor.consume_next_taxable_event` now handles `is_earning()` before calling the accounting method's seek. Pool-based methods (`moving_average`, `moving_average_at`) deplete the pool as a side effect of seeking, and that deduction was never restored for earnings, so a STAKING/INTEREST/MINING/AIRDROP event silently drained the Neu pool and overstated the gain on every later disposal. Added regression tests for both methods.
  * **Spekulationsfrist is now a calendar year, not 365 days.** `classify_disposal` compares Vienna calendar dates against the acquisition anniversary (§ 108 BAO) instead of `holding_days >= 365`, so a holding that spans a 29 February is no longer misreported as past the Frist; a disposal on the anniversary day is still inside the Frist (taxable). Classifiers (`classify_lot_regime`, Spekulationsfrist) now read `cost_basis_timestamp` so transferred Altvermögen keeps its original acquisition date. `classify_lot_regime` also follows the `from_lot` chain to the originating lot, so an explicit `at_regime=` marker survives a per-wallet transfer (the artificial "to" lot drops it from `notes`) instead of being overridden by the date fallback.
  * **Swap neutrality requires `moving_average_at`.** The native runner raises if a *Neuvermögen* outgoing swap leg yields no carried basis (i.e. the configured method is `fifo`/`moving_average`), instead of silently realizing a taxable gain on a row `classify_disposal` still buckets as `NEU_SWAP`. The guard only fires when the leg consumed Neu lots, so an *unmarked* swap row that `moving_average_at` legitimately routes to the Alt path (Alt swaps are taxable, no basis carry) is resolved without a carry rather than rejected.
  * **Per-wallet: same-timestamp artificial lots.** The accounting-engine AVL disambiguator is now sign-class aware, so a transferred ("to"-leg) lot with a negative id sorts above a real lot sharing its timestamp (matching list order). Previously the destination sell failed with "Total in-transaction crypto value < total taxable crypto value".
  * **open_positions partial cost basis.** The per-wallet cost-basis branch relies on the `InTransaction` invariant that `crypto_in` is positive; staking losses remain modeled as `OUT` STAKING transactions, so invalid zero-amount `IN` rows are rejected before report generation.
  * **`--asset` validation.** An unknown `--asset` now exits with a clear error instead of a raw `KeyError` traceback.
  * **Pool-mismatch diagnostic.** A Neuvermögen disposal whose `at_pool=<id>` has no lots while *other* Neu pools still hold lots now raises a specific, actionable `RP2ValueError` (the usual cause is funds acquired in one pool/wallet and disposed from another without re-tagging) instead of the cryptic "Total in-transaction crypto value < total taxable crypto value".
  * Simplification: unified `PoolAcquiredLotCandidates` onto a single per-pool sync cursor and moved the running-average / pool-depletion arithmetic onto the container (shared by both moving-average methods); `TransferAnalyzer.analyze` now reuses `InputData.create_all_transaction_set` instead of re-implementing it; removed fork-introduced dead code (`AccountingEngine.acquired_lot_2_partial_amount`, `rp2_main._uses_per_wallet_application`, `TaxEngineCursor.gain_loss_set`) and orphaned `mypy.ini` sections for the Phase-9-removed `tax_report_at`. Upstream surface (`get_next_taxable_event_and_amount`, `initialize`'s taxable-event iterator) was left intact to minimize merge friction.
  * Known limitation (per-wallet, draft): in per-wallet application an `IntraTransaction` crypto fee is realized as its own taxable disposal by `compute_tax` against the **accounting method's** lot selection, while `actual_amount` (used by `open_positions`) is tracked by the **transfer-semantics** pass. For a multi-lot transfer these can pick different lots, so `open_positions` realized + open basis may not reconcile to the original basis per-lot. This is structural to the inherited per-wallet draft (it predates this branch on the cross-wallet path) and is deferred to the per-wallet redesign; it does not affect Austrian users, who run universal application (where `open_positions` uses sold-percentage). An earlier attempt to special-case the self-transfer fee was reverted because it deducted the fee from the transfer's last lot rather than the accounting method's realization lot, which widened the per-lot discrepancy.
* Native Austrian swap carry ([PR #6](https://github.com/bitcoinaustria/rp2/pull/6)) — RP2 now computes the incoming leg's carried basis itself via `AT.compute_tax_for_assets` → `compute_native_at_tax` (new `src/rp2/plugin/country/at_native_tax_engine.py`), seeding the destination pool with `crypto_out_no_fee * neu_pool_avg_at_swap_time`. Kassiber emits both legs at market value and no longer synthesizes the carried value. Includes the per-pool sync-cursor fix that keeps a carried incoming lot from desyncing the destination pool.
* Hardened Austrian swap-link markers ([PR #4](https://github.com/bitcoinaustria/rp2/pull/4)) — cross-asset pairing validator (`validate_at_swap_link_pairing`): every `at_swap_link=<id>` must appear on exactly one `OutTransaction` and one `InTransaction` on two different assets, else `RP2ValueError`. Empty/duplicate/same-asset/backwards-in-time markers are rejected. Fee-aware zero-gain override for swap-neutral Neu `SELL` legs; non-`SELL` and Alt swap markers handled per § 27b Abs 3 Z 2.
* Per-wallet application (draft integration of [upstream PR #138](https://github.com/eprbell/rp2/pull/138)) — added `TransferAnalyzer`, `GlobalAllocator` (parsed only, not wired in at the universal → per-wallet boundary), and `per_wallet_tax_engine.compute_tax_per_wallet`. Added `[application_methods]` and `[transfer_methods]` config sections (parallel to `[accounting_methods]`), a `get_application_methods()` hook on `AbstractCountry`, and routing in `rp2_main` that selects the per-wallet path when `[application_methods]` picks `per_wallet`. US plugin opts into both `universal` and `per_wallet`. Current limitation: RP2 requires a single application method and a single transfer method for the whole run; year-boundary transitions (e.g. 2024 `universal` → 2025 `per_wallet`) are rejected explicitly. Transfer semantics defaults to the earliest configured accounting method when `[transfer_methods]` is omitted. Default behavior without `[application_methods]` is unchanged (universal).
* Phase 9 — moved Austrian report presentation to Kassiber. Removed `tax_report_at` generator, most `src/rp2/plugin/report/data/at/` templates, and the `de_AT` locale catalog; Austrian default generators shrink to `{open_positions}`, whose AT-specific template link now redirects to the generic template. Added the stable Kassiber-facing classification surface on `rp2.plugin.country.at`: `AtDisposalCategory` enum + `classify_disposal(GainLoss) -> AtDisposalCategory` capture the semantic Alt/Neu/swap/earn routing previously embedded inside the report generator, so Kassiber does not re-implement Austrian tax semantics when rendering E 1kv. Rolls back the rp2-side pieces of Phases 5 and 6; tax math (Phases 1–4, 7, 8) is unchanged. The STAKING/INTEREST → `INCOME_CAPITAL_YIELD` semantic split introduced in Phase 7 is preserved inside `classify_disposal` — the form-code mapping to Kz 175 now lives in Kassiber.
...
* Phase 5 — ~~added `tax_report_at` generator.~~ **Rolled back in Phase 9.** Originally emitted a FinanzOnline-aligned summary with Kennzahlen 172/174/175/176/801; ownership moved to Kassiber as part of the engine/presentation split.
* Phase 6 — ~~added `de_AT` localization catalog for the Austrian report.~~ **Rolled back in Phase 9** together with the generator it served.
* Phase 7 — review followups. `at_pool=<id>` marker implemented (partitions the Neu moving-average pool); removed silent "alt-first" fallback on unmarked mixed-pool disposals (raises `RP2ValueError`); empty `at_swap_link=` now rejected; `rp2_full_report` dropped from AT default generators (its long/short split is misleading when `long_term_capital_gain_period = sys.maxsize`); sharpened the Kassiber handoff contract in AGENTS.md. The Kz 175 routing introduced here migrated into `classify_disposal` during Phase 9; the STAKING/INTEREST → `INCOME_CAPITAL_YIELD` split is preserved as engine-level semantics.
...
* Phase 4 follow-up — Austrian `moving_average_at` swap-neutral outgoing legs now derive `unit_cost_basis_override` from fee-aware taxable proceeds (`fiat_taxable_amount / crypto_balance_change`) rather than raw `spot_price`, so `NEU_SWAP` rows stay at exactly zero gain even when the disposal carries a crypto fee. Updated `AGENTS.md` and `README.dev.md` to document the fee-aware override.

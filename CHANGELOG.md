...
* Correctness fixes (review pass):
  ...
  * **Per-wallet: self-transfer crypto fee.** The fee genuinely leaves the wallet on a self-transfer; it is now re-deducted from the final lot after the principal is restored, so remaining balances (open_positions, GlobalAllocator) no longer overstate by the fee.

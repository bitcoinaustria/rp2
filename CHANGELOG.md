...
* Correctness fixes (review pass):
  ...
  * **open_positions division guard.** The per-wallet cost-basis branch no longer divides by `crypto_in` when it is zero (allowed for STAKING), avoiding a `DivisionByZero` during report generation.

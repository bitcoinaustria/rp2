...
**ABOUT THIS FORK**: This is the Kassiber-maintained [`bitcoinaustria/rp2`](https://github.com/bitcoinaustria/rp2) fork. In addition to upstream's features it adds Austrian crypto tax support (`rp2_at`, `moving_average_at` with § 27b EStG Alt/Neu split and swap neutrality via `notes` markers) and a draft integration of the per-wallet application work from [upstream PR #138](https://github.com/eprbell/rp2/pull/138). See [AGENTS.md](AGENTS.md) for the fork roadmap and the Kassiber handoff surface, [CHANGELOG.md](CHANGELOG.md) for phase-by-phase history, and [docs/supported_countries.md](docs/supported_countries.md) for per-country feature support. The BMF-aligned E 1kv / FinanzOnline presentation for Austrian users lives in Kassiber, not in `rp2_at`.

**IMPORTANT NOTICE FOR USA TAXPAYERS**: Starting in 2025, the USA has transitioned from [Universal Application to Per-Wallet Application](https://github.com/eprbell/rp2/issues/135#issuecomment-2454959334). This fork ships a draft per-wallet path (see [upstream PR #138](https://github.com/eprbell/rp2/pull/138) and [docs/supported_countries.md](docs/supported_countries.md#usa)), but it requires a single application method for the whole run: mid-run year-boundary transitions (e.g. 2024 `universal` → 2025 `per_wallet`) are rejected explicitly, and `GlobalAllocator` is parsed-only. Audit results before relying on it for 2025+ USA tax reporting — upstream tracking remains at [issue #135](https://github.com/eprbell/rp2/issues/135).

**IMPORTANT NOTICE FOR TAXPAYERS WORLDWIDE**: Per-wallet application is country-specific and opt-in. In this fork it is enabled for the US plugin only and is limited to a single application method per run (see the USA notice above). If your country mandates per-wallet application with year-boundary switching in the same run, that is not yet supported — track [issue 135](https://github.com/eprbell/rp2/issues/135#issuecomment-2454959334). If your country permits Universal Application (already supported by RP2), you may safely use RP2.
...
The RP2 executable is country-dependent: `rp2_<country_code>`, where country code is a [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2), 2-letter identifier (e.g. `rp2_us`, `rp2_at`, etc).

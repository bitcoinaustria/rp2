...
* accounting schedules may switch from a lot-based method to a pool-based method (`moving_average` or `moving_average_at`). Consecutive entries naming the same pool method preserve its running average. Switching from a pool-based method to a different method is rejected because historical acquisition costs cannot reconstruct the remaining pool basis;

...
from rp2.rp2_decimal import CRYPTO_DECIMALS, ZERO, RP2Decimal
from rp2.rp2_error import RP2Error, RP2RuntimeError, RP2ValueError
...
def _create_and_process_transaction(
    ...
    if isinstance(transaction, InTransaction) and transaction.is_crypto_fee_defined:
        ...
        if configuration.country.country_iso_code == "at":
            # AT notes carry accounting instructions: keep the acquisition's tokens intact,
            # and charge its regime/pool for the fee without duplicating the swap-link id.
            # Import lazily because the AT entry point imports this parser via rp2_main.
            ...
            fee_notes = f"at_regime={classify_lot_regime(transaction)} {AT_POOL_MARKER}{pool_id_from_notes(transaction.notes)} {fee_notes}"
...
def _get_decimal_constructor_argument_names(class_name: str) -> List[str]:
    ...
    if class_name not in globals():
        raise RP2RuntimeError(f"Internal error: couldn't find class {class_name}")
    class_to_inspect = globals()[class_name]
    if not issubclass(class_to_inspect, AbstractTransaction):
        raise RP2RuntimeError(f"Internal error: class {class_name} is not a subclass of AbstractTransaction")
...
def _process_constructor_argument_pack(
    ...
    for numeric_parameter in numeric_parameters:
        if numeric_parameter in argument_pack:
            value = argument_pack[numeric_parameter]
            if value == "__unknown":
                # Preserve the actionable DaLI diagnostic instead of wrapping it as a bad number.
                unresolved_arguments = {key: argument for key, argument in argument_pack.items() if key != "configuration"}
                raise RP2RuntimeError(
                    f"Encountered an unresolved DaLI transaction (read DaLI's documentation / FAQ to learn how to resolve this issue): {unresolved_arguments}"
                )
            try:
                if isinstance(value, bool):
                    raise ValueError("Boolean cells are not transaction amounts")
                # ezodf exposes numeric cells as floats. Format them to RP2's supported
                # crypto precision; the former 11-place limit rounded valid amounts to zero.
                if value is None:
                    argument_pack[numeric_parameter] = None
                else:
                    decimal_value = RP2Decimal(f"{value:.{CRYPTO_DECIMALS}f}")
                    # Retain the historical scale where it loses no precision, so generated
                    # fee descriptions and existing report fixtures keep their spelling.
                    legacy_value = RP2Decimal(f"{value:.11f}")
                    argument_pack[numeric_parameter] = legacy_value if decimal_value == legacy_value else decimal_value

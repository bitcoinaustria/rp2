...
LOGGER: logging.Logger = create_logger("tax_report_ie")
...
class Generator(AbstractODSGenerator):
    MIN_ROWS: int = 20
    MAX_COLUMNS: int = 20
    OUTPUT_FILE: str = "tax_report_ie.ods"
    ...
    def generate(
        ...
        template_path: str = self._get_template_path("tax_report_ie", country, generation_language)
    ...
    def __generate(self, output_file: Any, asset: str, gain_loss_set: GainLossSet, row_indexes: Dict[str, int]) -> None:
        ...
        for entry in gain_loss_set:
            ...
            self._fill_cell(sheet, row_index, 3, gain_loss.taxable_event.timestamp.strftime("%Y/%m/%d"), visual_style=taxable_event_note_vs)
            ...
            if gain_loss.acquired_lot:
                ...
                self._fill_cell(sheet, row_index, 2, gain_loss.acquired_lot.timestamp.strftime("%Y/%m/%d"), visual_style=acquired_lot_note_vs)

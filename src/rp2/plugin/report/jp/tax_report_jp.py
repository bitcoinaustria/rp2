...
class Generator(AbstractODSGenerator):
    ...
    def __generate_asset(self, computed_data: ComputedData, output_file: Any) -> None:
        ...
        for year, transaction_set in years_2_transaction_sets.items():
            ...
            self._fill_cell(
                summary_sheet,
                self.__year_row_offset[year] + 2,
                1,
                self._formula(f"=SUM(B8:B{self.__year_row_offset[year] + 2})"),
                apply_style=False,
            )
            # Gift Received
            self._fill_cell(
                ...
                self.__year_row_offset[year] + 2,
                2,
                self._formula(f"=SUM(C8:C{self.__year_row_offset[year] + 2})"),
                apply_style=False,
            )
            # Total Amount in yen
            self._fill_cell(
                ...
                self.__year_row_offset[year] + 2,
                5,
                self._formula(f"=SUM(F8:F{self.__year_row_offset[year] + 2})"),
                apply_style=False,
            )
            # Net Income Amt
            self._fill_cell(
                ...
                self.__year_row_offset[year] + 2,
                6,
                self._formula(f"=SUM(G8:G{self.__year_row_offset[year] + 2})"),
                apply_style=False,
            )
    ...
    def __generate_asset_year(self, asset: str, year: int, transaction_list: List[AbstractTransaction], output_file: Any, previous_year_row_offset: int) -> int:
        ...
        self._fill_cell(asset_year_sheet, row_index + 2, 4, self._formula(f"=IF(SUM(E22:E{row_index+2})=0;0;SUM(E22:E{row_index+2}))"), apply_style=False)
        self._fill_cell(asset_year_sheet, row_index + 2, 5, self._formula(f"=IF(SUM(F22:F{row_index+2})=0;0;SUM(F22:F{row_index+2}))"), apply_style=False)
        self._fill_cell(asset_year_sheet, row_index + 2, 6, self._formula(f"=IF(SUM(G22:G{row_index+2})=0;0;SUM(G22:G{row_index+2}))"), apply_style=False)
        self._fill_cell(asset_year_sheet, row_index + 2, 7, self._formula(f"=IF(SUM(H22:H{row_index+2})=0;0;SUM(H22:H{row_index+2}))"), apply_style=False)
        self._fill_cell(asset_year_sheet, row_index + 2, 8, self._formula(f"=IF(SUM(I22:I{row_index+2})=0;0;SUM(I22:I{row_index+2}))"), apply_style=False)
        ...
        self._fill_cell(asset_year_sheet, row_index + 8, 5, self._formula(f"=E13+E{row_index+3}"), apply_style=False)
        self._fill_cell(asset_year_sheet, row_index + 9, 5, self._formula(f"=F13+F{row_index+3}"), apply_style=False)
        ...
        self._fill_cell(
            ...
            row_index + 9,
            6,
            self._formula(f"=IF((E{row_index+9}+F{row_index+9});(E{row_index+10}+F{row_index+10})/(E{row_index+9}+F{row_index+9});0)"),
            apply_style=False,
        )

        ## This year's total sales, etc...
        # Total units sold
        self._fill_cell(asset_year_sheet, row_index + 8, 7, self._formula(f"=G13+G{row_index+3}"), apply_style=False)
        # Total in yen (total units * average unit price)
        self._fill_cell(asset_year_sheet, row_index + 9, 7, self._formula(f"=H{row_index+9}*G{row_index+10}"), apply_style=False)

        ## This year's totals
        # Total units left
        self._fill_cell(asset_year_sheet, row_index + 8, 8, self._formula(f"=E{row_index+9}+F{row_index+9}-H{row_index+9}"), apply_style=False)
        # Total in yen (total units left * average unit price)
        self._fill_cell(asset_year_sheet, row_index + 9, 8, self._formula(f"=I{row_index+9}*G{row_index+10}"), apply_style=False)

        ### 5 Calculation of Income Amount for Virtual Currency (仮想通貨の所得金額の計算)

        ## Gross Income Amount
        # Sale Price
        self._fill_cell(asset_year_sheet, row_index + 17, 0, self._formula(f"=H13+H{row_index+3}"), apply_style=False)
        # Cost of Sale
        self._fill_cell(asset_year_sheet, row_index + 17, 5, self._formula(f"=H{row_index+10}"), apply_style=False)
        # Total Fees
        self._fill_cell(asset_year_sheet, row_index + 17, 6, self._formula(f"=I{row_index+3}"), apply_style=False)
        # Net Income Amt
        self._fill_cell(asset_year_sheet, row_index + 17, 8, self._formula(f"=I{row_index+20}-I{row_index+21}"), apply_style=False)

        # Total Profit
        self._fill_cell(asset_year_sheet, row_index + 19, 8, self._formula(f"=ROUNDDOWN(SUM(A{row_index+18}:E{row_index+18});0)"), apply_style=False)
        # Total Necessary Expenses
        self._fill_cell(asset_year_sheet, row_index + 20, 8, self._formula(f"=ROUNDUP(SUM(F{row_index+18}:H{row_index+18});0)"), apply_style=False)
        ...
        quoted_tax_sheet_name: str = self._quote_formula_sheet_name(self.get_tax_sheet_name(asset, year))
        self._fill_cell(year_summary_sheet, self.__year_row_offset[year], 3, self._formula(f"={quoted_tax_sheet_name}.G{row_index+10}"), apply_style=False)
        # Formula for End Balance Crypto
        self._fill_cell(year_summary_sheet, self.__year_row_offset[year], 4, self._formula(f"={quoted_tax_sheet_name}.I{row_index+9}"), apply_style=False)
        # Formula for End Balance Yen
        self._fill_cell(year_summary_sheet, self.__year_row_offset[year], 5, self._formula(f"={quoted_tax_sheet_name}.I{row_index+10}"), apply_style=False)
        # Formula for Net Income Amt
        self._fill_cell(year_summary_sheet, self.__year_row_offset[year], 6, self._formula(f"={quoted_tax_sheet_name}.I{row_index+18}"), apply_style=False)

...
class TestGainLoss(unittest.TestCase):
  ...
  acquired_lot=InTransaction:
    ...
    def test_gain_loss_equality_and_hashing(self) -> None:
        ...
        gain_loss7: GainLoss = GainLoss(self._configuration, RP2Decimal("0.001"), self._intra, self._in_buy, unit_cost_basis_override=RP2Decimal("1"))
        gain_loss8: GainLoss = GainLoss(self._configuration, RP2Decimal("0.001"), self._intra, self._in_buy, unit_cost_basis_override=RP2Decimal("1"))
        gain_loss9: GainLoss = GainLoss(self._configuration, RP2Decimal("0.001"), self._intra, self._in_buy, unit_cost_basis_override=RP2Decimal("2"))
        ...
        self.assertNotEqual(gain_loss, gain_loss7)
        self.assertEqual(gain_loss7, gain_loss8)
        self.assertNotEqual(gain_loss7, gain_loss9)
        self.assertEqual(hash(gain_loss), hash(gain_loss))
        self.assertEqual(hash(gain_loss), hash(gain_loss2))
        self.assertEqual(hash(gain_loss7), hash(gain_loss8))
        ...
        self.assertNotEqual(hash(gain_loss), hash(gain_loss7))
        self.assertNotEqual(hash(gain_loss7), hash(gain_loss9))
        self.assertEqual(gain_loss.internal_id, "30->10")
        self.assertEqual(gain_loss7.internal_id, "30->10@unit_cost_basis_override=1.0000000000000")

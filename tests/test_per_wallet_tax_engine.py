...
class TestPerWalletTaxEngine(unittest.TestCase):
    ...
    def test_per_wallet_matches_universal_without_transfers(self) -> None:
        ...
        in1 = InTransaction(self._configuration, "2020-01-01 08:00:00 +0000", "B1", "Coinbase", "Bob", "Buy",
                            spot_price=RP2Decimal("10000"), crypto_in=RP2Decimal("1.0"),
                            fiat_in_no_fee=RP2Decimal("10000"), fiat_in_with_fee=RP2Decimal("10050"), fiat_fee=RP2Decimal("50"),
                            unique_id="1", row=1)
        in2 = InTransaction(self._configuration, "2020-06-01 08:00:00 +0000", "B1", "Coinbase", "Bob", "Buy",
                            spot_price=RP2Decimal("20000"), crypto_in=RP2Decimal("1.0"),
                            fiat_in_no_fee=RP2Decimal("20000"), fiat_in_with_fee=RP2Decimal("20100"), fiat_fee=RP2Decimal("100"),
                            unique_id="2", row=2)
        out1 = OutTransaction(self._configuration, "2021-03-01 08:00:00 +0000", "B1", "Coinbase", "Bob", "Sell",
                              spot_price=RP2Decimal("30000"), crypto_out_no_fee=RP2Decimal("0.5"), crypto_fee=RP2Decimal("0"),
                              unique_id="3", row=3)
        ...
        universal_totals = {
            (y.year, y.transaction_type, y.is_long_term_capital_gains): (y.crypto_amount, y.fiat_gain_loss)
            for y in universal_computed.yearly_gain_loss_list
        }
        per_wallet_totals = {
            (y.year, y.transaction_type, y.is_long_term_capital_gains): (y.crypto_amount, y.fiat_gain_loss)
            for y in per_wallet_computed.yearly_gain_loss_list
        }
    ...
    def test_per_wallet_runs_end_to_end_with_transfers(self) -> None:
        ...
        in_cb = InTransaction(self._configuration, "2020-01-01 08:00:00 +0000", "B1", "Coinbase", "Bob", "Buy",
                              spot_price=RP2Decimal("10000"), crypto_in=RP2Decimal("2.0"),
                              fiat_in_no_fee=RP2Decimal("20000"), fiat_in_with_fee=RP2Decimal("20100"), fiat_fee=RP2Decimal("100"),
                              unique_id="1", row=1)
        in_kr = InTransaction(self._configuration, "2020-01-02 08:00:00 +0000", "B1", "Kraken", "Alice", "Buy",
                              spot_price=RP2Decimal("10000"), crypto_in=RP2Decimal("1.0"),
                              fiat_in_no_fee=RP2Decimal("10000"), fiat_in_with_fee=RP2Decimal("10050"), fiat_fee=RP2Decimal("50"),
                              unique_id="2", row=2)
        intra = IntraTransaction(self._configuration, "2020-06-01 08:00:00 +0000", "B1",
                                 from_exchange="Coinbase", from_holder="Bob",
                                 to_exchange="Kraken", to_holder="Alice",
                                 spot_price=RP2Decimal("15000"), crypto_sent=RP2Decimal("1.0"), crypto_received=RP2Decimal("1.0"),
                                 unique_id="3", row=3)
        out_kr = OutTransaction(self._configuration, "2021-03-01 08:00:00 +0000", "B1", "Kraken", "Alice", "Sell",
                                spot_price=RP2Decimal("30000"), crypto_out_no_fee=RP2Decimal("1.5"), crypto_fee=RP2Decimal("0"),
                                unique_id="4", row=4)
        ...
        per_wallet_computed: ComputedData = compute_tax_per_wallet(
            self._configuration, self._accounting_engine, self._transfer_semantics, input_data
        )
